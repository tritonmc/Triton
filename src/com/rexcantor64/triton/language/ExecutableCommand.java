...
public class ExecutableCommand {
    ...
    public static ExecutableCommand parse(String input) {
        ...
        if (inputSplit.length < 2) {
            MultiLanguagePlugin.get().logWarning("Language command '%1' doesn't have a type. Using type 'PLAYER' by default.", input);
        ...
        if (type == null) {
            MultiLanguagePlugin.get().logWarning("Language command '%1' has invalid type '%2'. Using the default type 'PLAYER'.", input, inputSplit[0]);
            type = Type.PLAYER;
        }
        if (inputSplit.length < 3 || !MultiLanguagePlugin.isBungee())
