    ...
    public void rerender(boolean force) {
        ...
        for (int i = 0; i < 15; i++) {
            ...
            if (i >= topEntries.size()) {
                if (line.score != null) {
                    line.score = null;
                    bridge.updateEntryScore(line.entry + (bridge.useComponents() ? "" : ScoreboardUtils.getEntrySuffix(i)), null);
                }
                continue;
            }
            ...
            if (!force && entry.equals(line.rawEntry) && (bridge.useComponents() ? (team.getPrefixComp() == line.rawPrefixComp && team.getSuffixComp() == line.rawSuffixComp) : (team.getPrefix().equals(line.rawPrefix) && team.getSuffix().equals(line.rawSuffix)))) {
